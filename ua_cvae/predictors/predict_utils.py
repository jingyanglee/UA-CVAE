...
def beam_search_UA(input_ids, type_ids, vocab, model, max_len, beam_size, latent_sample=None, logvar = None):
    with torch.no_grad():
        ...
        for i in range(max_len):
            ...
            logits = model.decoder(input_ids=beam_input_ids, type_ids=beam_type_ids,
                                   latent_sample=beam_latent_sample, logvar=logvar)[:, -1, :]
...
def beam_search_Sep(input_ids, type_ids, vocab, model, max_len, beam_size, latent_sample=None, logvar = None):
    with torch.no_grad():
        batch_size = input_ids.shape[0]
        device = input_ids.device
        ...
        if logvar is None:
            ...
            beam_logvar = logvar.squeeze(1).repeat(1, beam_size, 1).view(-1, latent_sample.shape[-1])
        ...
        for i in range(max_len):
            ...
            logits = model.decoder(input_ids=beam_input_ids, type_ids=beam_type_ids,
                                   latent_sample=beam_latent_sample, logvar=beam_logvar)[:, -1, :]
